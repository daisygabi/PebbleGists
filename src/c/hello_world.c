...
static void update_time() {
  ...
  static char s_day_buffer[125];
  
  // Get current day
  strftime(s_day_buffer, sizeof(s_day_buffer), "%A", tick_time);

  // Words explaining each day
  static char* strings[7] = {"\nGrass doesn\'t even grow", "\nHope for the best", "\nHere we go again", 
                             "\nAlmost there", "\nDrink responsibly", "\nHangover loading", "\nJust before Monday"};  
  // Copy apropriate text
  strcat(s_day_buffer, strings[tick_time->tm_wday]);
  
  // Display day on the TextLayer
...
static void main_window_load(Window *window) {
  ...
  s_day_layer = text_layer_create(
     GRect(00, PBL_IF_ROUND_ELSE(58, 52), bounds.size.w, 80));
  ...
  // Improve layout of day details
  text_layer_set_background_color(s_day_layer, GColorClear);
  text_layer_set_text_color(s_day_layer, GColorRed);
  text_layer_set_text(s_day_layer, "Freaking");
